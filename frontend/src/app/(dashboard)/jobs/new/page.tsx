  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6 animate-[fadeIn_0.2s_ease-in-out]">
      <div className="flex items-center gap-2 sm:gap-3">
        <Link href="/jobs" className="p-2 hover:bg-gray-100 rounded-lg transition-colors shrink-0">
          <ArrowLeft className="w-5 h-5 text-gray-600" />
        </Link>
        <div className="min-w-0">
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900 tracking-tight">Create New Job</h1>
          ...
          {form.requiredSkills.length > 0 && (
            <div className="space-y-2">
              {form.requiredSkills.map((skill, idx) => (
                <div key={idx} className="flex flex-wrap items-center gap-2 sm:gap-3 bg-gray-50 rounded-lg px-3 py-2">
                  <GripVertical className="w-4 h-4 text-gray-300 hidden sm:block" />
                  <span className="text-sm font-medium text-gray-700 flex-1 min-w-0 truncate">{skill.name}</span>
